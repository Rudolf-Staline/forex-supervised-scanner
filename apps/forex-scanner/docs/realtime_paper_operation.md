...
The validation is local-only and Windows/MT5-dependent. It checks MT5 package import, terminal initialization, account and terminal info reads, symbol resolution and selection, latest requested timeframe candles, latest tick, timestamp-normalized candle age, spread, spread/ATR when possible, missing bars, duplicate bars, provider-data latency (`provider_latency_ms`) separately from MT5 request latency (`latency_ms`), and bounded repeated polling. It validates market-data readiness only; it does not authorize live trading, does not call `order_send`, does not submit broker orders, does not mutate `.env`, does not run as a daemon, and does not add live broker execution capability.
